 ...
 *Author: Thomas Krahulik (Stony Brook University)
   *Modified 4/11/2017 by Josh LaBounty to produce .txt output
	...
	int nEvents = 10000 )
{
	...
	TTree* tree = new TTree("EICTree", "Pythia 6 tree");
	...
	myfile << "  Pythia Output File" << endl;
	myfile << " ============================================" << endl;
	myfile << "  I, ievent, genevent, subprocess, nucleon,              targetparton, xtargparton, beamparton, xbeamparton,               thetabeamprtn, truey, trueQ2, truex, trueW2, trueNu,              leptonphi, s_hat, t_hat, u_hat, pt2_hat, Q2_hat, F2, F1, R,        sigma_rad, SigRadCor, EBrems, photonflux, nrTracks" << endl;
	myfile << " ============================================" << endl;
	myfile << "  I\tKS\tKF(ID)\tORIG\tD1\tD2\tpx\tpy\tpz\tE\tm\tvx\tvy\tvz" << endl;
	myfile << " ============================================" << endl;
	...
	for (Int_t i = 0; i < nEvents; i++) 
	{
		...
		tree->Fill();
		myfile << "  0   " << i << "     1   95 2212         21  0.000000  21 0.000000     0.000000      0.03587194790                       0.00016131040 0.00000022484      718.31915283203                  382.32009887695                                                     0.00773132309        0.00000000000           0.000000000          0.000000000           0.000000000           0.000000000           0.000000000           0.000000000           0.000000000           0.000000000           0.000000000           0.000000000           1.422321899             670" << endl;
		myfile << " ============================================" << endl;
		...
		for(int j = 0; j < nPart; j++)
		{
			...
			myfile << "    ";
			myfile << j+1 << "    ";
			myfile << ((TMCParticle*)p->At(j))->GetKS() << "    ";
			myfile << ((TMCParticle*)p->At(j))->GetKF() << "    ";
			myfile << ((TMCParticle*)p->At(j))->GetParent() << "    ";
			myfile << ((TMCParticle*)p->At(j))->GetFirstChild() << "    ";
			myfile << ((TMCParticle*)p->At(j))->GetLastChild() << "    ";
			myfile << ((TMCParticle*)p->At(j))->GetPx() << "    ";
			myfile << ((TMCParticle*)p->At(j))->GetPy() << "    ";
			myfile << ((TMCParticle*)p->At(j))->GetPz() << "    ";
			myfile << ((TMCParticle*)p->At(j))->GetEnergy() << "    ";
			myfile << ((TMCParticle*)p->At(j))->GetMass() << "    ";
			myfile << ((TMCParticle*)p->At(j))->GetVx() << "    ";
			myfile << ((TMCParticle*)p->At(j))->GetVy() << "    ";
			myfile << ((TMCParticle*)p->At(j))->GetVz();
			myfile << endl;
		}
